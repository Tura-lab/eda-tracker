...
export default function AddExpenseModal({ isOpen, onClose, onSuccess }: AddExpenseModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSplit) {
        // Split the amount among all selected people (plus the current user if includeMe is true)
      ...
      const transactions = selectedUsers.map(user => ({
        ...
        description: isSplit 
          ? includeMe
            ...
            : `${description} (Total: ${parseFloat(amount).toFixed(2)} ETB split among ${selectedUsers.map(u => getFirstName(u.name)).join(', ')})`
          : description,
    ...
    <div className="fixed inset-0 backdrop-blur-sm bg-black/50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-4 sm:p-6 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <label className="flex items-center space-x-3 p-3 rounded-md border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (e.target.checked) {
                      setIncludeMe(true) // Reset includeMe to true when enabling split
                    }
                  }}
                  className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 dark:border-gray-600 rounded"
                />
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Divide the total amount among selected people.
                  </p>
                </div>
              </label>
            </div>
          ...
          {/* Include Me Checkbox - Only shown when Split is checked */}
          {isSplit && selectedUsers.length > 0 && (
            <div>
              <label className="flex items-center space-x-3 p-3 rounded-md border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700/50">
                <input
                  type="checkbox"
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Include yourself in the split. Uncheck to split only among selected people.
                  </p>
                </div>
              </label>
            </div>
